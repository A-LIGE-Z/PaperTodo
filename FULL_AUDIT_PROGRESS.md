...
- [x] `dotnet build PaperTodo.csproj -c Release`
  - 证据：本轮阶段 8 复跑 -> 0 warning / 0 error，输出 `输出\PaperTodo-v2.0\PaperTodo.dll`。
- [x] `git diff --check`
  - 证据：本轮阶段 8 复跑 -> 无空白错误。
- [x] 资源 key parity
  - 证据：`Strings.en.resx: missing=0 extra=0`；`Strings.ja.resx: missing=0 extra=0`；`Strings.ko.resx: missing=0 extra=0`。
- [ ] 单屏基础手测
- [ ] 多屏同 DPI 手测
- [ ] 多屏混合 DPI 手测
- [ ] 托盘菜单手测
- [ ] 待办 / 笔记核心路径手测
- [ ] 退出保存 / 重启恢复手测
- [x] 启动命令手测：`show` / `hide` / `toggle` / `new-todo` / `new-note` / `exit`
  - 证据：隔离复制 Release 输出到 `.audit-runtime\stage8-command` 后运行；`exit` 空数据无 crash；`new-todo` -> `papers=1 types=todo`；`new-note` -> `papers=1 types=note`；`show` hidden -> visible；`hide` visible -> hidden；`toggle` visible -> hidden；所有进程 `ExitCode=0`，`leftoverProcesses=0`。
...
- [x] 读取蒸馏文件
  - 证据：已读取 `F:\project\NEW__当前项目\用户 - 副本.txt`。本阶段只提取产品 / 协作约束，不把人格材料写入项目文档。
- [x] 是否仍符合“桌面上的几张纸”
  - 结论：仍符合。当前功能仍围绕桌面纸片、待办、便签、轻量 Markdown、托盘和胶囊显示；没有账号、同步、集中管理页、统计、日历、标签、归档等越界功能。
- [x] 是否有功能膨胀
  - 结论：本轮多队列贴边胶囊和收起全部增加了实现复杂度，但仍服务于“纸片在桌面边缘如何占位 / 显隐”的核心交互，不是产品域膨胀。继续禁止把它发展成任务管理器、知识库或配置中心。
- [x] 是否把实现复杂度转嫁给用户
  - 结论：主要风险在贴边胶囊。当前已通过默认继承队列、拖出阈值、主胶囊拖动起始高度、关闭模式清理状态、动画开关门禁降低用户理解成本；剩余要求是多屏 / 混合 DPI 必须实测，不能让用户理解 monitor key、slot、per-queue margin 这些内部概念。
- [x] 哪些应该砍、留、延后
  - 留：胶囊、贴边多队列、收起全部、待办关联笔记、轻量 Markdown、全屏避让、托盘控制，这些都直接服务当前产品形态。
  - 延后：Scoop / Choco 渠道、透明度常态设置、更多 Markdown 能力、统计 / 搜索 / 标签 / 分类。
  - 砍：任何需要用户维护结构化体系的集中页、账号同步、复杂提醒日历、块级编辑器、附件 / 嵌入内容。

## 阶段 10：最终报告

- [x] 高 / 中 / 低风险问题清单
  - 高风险开放问题：本轮代码审计未留下明确高风险开放 bug。
  - 中风险残余：多屏同 DPI、多屏混合 DPI、跨屏拖拽、125% / 150% DPI 右侧贴边漏白只能由真实环境最终证明；当前机器只有单屏 100% DPI。
  - 中风险残余：关闭动画开关后不再启动新动画，但不会强制终止设置切换瞬间已经在跑的所有行级 / 主题动画。
  - 低风险残余：200ms topmost 刷新 timer 有固定窗口管理成本，阶段 5 判断当前可接受。
- [x] 已修复问题清单
  - A001 backup 恢复后首次保存保护。
  - A002 右侧贴边主胶囊宽度变化保持贴边。
  - A003 贴边胶囊失去资格后恢复为可见普通窗口。
  - A004 旧全局贴边起始高度按原显示器规范化。
  - A005 超长旧笔记初始加载也受长度保护。
  - A006 贴边激活临时 slot 坐标不写回普通几何。
  - A007 超长旧笔记延后截断，避开 AvalonEdit undo group 异常。
  - A008 关闭动画后贴边胶囊 / 主胶囊路径不再启动新动画。
- [x] 已排除风险清单
  - 数据恢复不会在 strict parse 失败时用空状态覆盖旧文件；backup 恢复保留失败主文件和恢复用 backup。
  - 普通纸片几何与胶囊 / 贴边临时几何有保存边界。
  - 单实例后续进程转发命令并退出；`exit` 空数据不创建默认纸片。
  - 托盘图标保留 `IconSource = LoadTrayIconSource()`，菜单打开时重建。
  - 笔记编辑 / 预览仍共用 `MarkdownTextBox`，没有拆成双控件。
  - 用户可见更新已写入 `CHANGELOG.md` 的 `### Unreleased`。
- [x] 结构债清单
  - `AppController` 和 `PaperWindow` partial 总体仍很大，胶囊状态横跨 controller / window / master；当前靠文档化状态轴和集中 helper 控风险。
  - 贴边胶囊状态轴多：slot、visual、gesture、open origin、collapse-all retracted、per-queue margin；未来新增行为必须先进状态表，不应直接塞局部布尔值。
  - WPF UI 自动化覆盖不足，真实托盘 / 多屏 / DPI 仍依赖手测。
- [x] 性能判断
  - 未发现新的持续 CPU 热点。保存防抖、Markdown 长度上限、批量待办动画上限、托盘按需重建和主题低频刷新都在可接受范围。
  - 主要残余成本是 topmost 刷新 timer 与交互瞬间的透明窗口移动；当前换取置顶 / 全屏避让稳定性，接受。
- [x] 视觉和动画判断
  - 右侧贴边漏白、主胶囊宽度变化离边、滑回先变窄、拖拽态过小、关闭动画后仍动这些已在代码层修正或复核。
  - 当前视觉最终风险不在代码推演，而在真实多屏 / 混合 DPI / 不同任务栏位置下的像素级观感。
- [x] 发布前必修清单
  - 若发 `rc`：当前代码审计和自动回归支持发布；建议用户再用真实多屏 / 混合 DPI 做一轮贴边 hover、拖出、收起全部、隐藏全部、托盘删除确认。
  - 若发正式版：必须补齐多屏同 DPI、多屏混合 DPI、125% / 150% DPI、托盘真实点击和核心待办 / 笔记手测记录。
- [x] 发布后可缓修清单
  - 降低 topmost timer 的无事刷新频率。
  - 继续拆小 `AppController` / `PaperWindow` 的低风险 UI 构建代码。
  - 给启动命令 / StateStore / MarkdownTextBox 增加自动化测试壳。
- [x] 最终建议：能否发 rc，能否发正式版，剩余风险是什么
  - 建议：可以发 `2.0rc2` 浏览 / 测试版。正式版不建议在缺少真实多屏 / 混合 DPI 手测记录时直接发布。
  - 剩余风险：多显示器几何和托盘真实点击仍是人机环境相关风险；代码审计能降低但不能替代最终手测。
