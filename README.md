...
| 多张独立纸片直接停留在桌面上，待办和笔记各自保存位置、尺寸与状态。 | 笔记纸支持 Markdown 编辑 / 浏览双模式，浏览态保留轻量排版并弱化标记干扰。 |

| 深色模式 | 胶囊模式 |
| --- | --- |
| ![深色模式](screenshots/Dark%20Mode.jpg) | ![胶囊模式](screenshots/Pill%20Mode.gif) |
| 深色模式保持纸片质感，同时降低夜间桌面的视觉刺激。 | 纸片可折叠为小胶囊，减少桌面占用，并保留快速展开入口。 |

| 胶囊自动贴边 |
| --- |
| ![胶囊自动贴边](screenshots/Pill%20Edge.gif) |
| 折叠胶囊可自动排列到屏幕右侧边缘，常态半隐藏，悬浮时滑出。 |
