...
云端 Release 发布两个 Windows x64 单文件：自包含 .NET Runtime 的压缩单文件，以及不带运行库的 no-runtime 单文件。本地打包只生成自包含压缩单文件 `self-contained-small.exe`。WPF 版本不要开启 `PublishTrimmed` 或 Native AOT。
