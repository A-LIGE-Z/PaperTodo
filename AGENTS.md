...
Markdown 只做轻量显示和编辑辅助，不扩展图片、表格、HTML、附件、嵌入内容或完整块编辑器。
...
正式发布保留两类 Windows x64 单文件：带运行库自包含版、无运行库轻量版。WPF 版本不要开启 `PublishTrimmed` 或 Native AOT。
