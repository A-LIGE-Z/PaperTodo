...
<InformationalVersion>1.6</InformationalVersion>
```
...
7. 调用 `AppController.Start(createDefaultPaper: !startupCommand.CreatesPaper)`。
8. 执行当前进程的启动命令。
9. 启动命名管道监听，后续实例启动时会把参数转发给主实例执行；无参数的后续实例默认按 `show` 处理。
...
- 常态可见宽度是 `CapsuleWidth / 2 + 10`，比半隐藏更往屏幕内一点；悬浮滑出时右侧会越过屏幕边缘 `8px`，让弹出态不显得贴得太死。
...
```powershell
dotnet publish .\PaperTodo.csproj -c Release -r win-x64 --self-contained true -o 输出\PaperTodo-v1.6-win-x64-self-contained-compressed\ -p:PublishSingleFile=true -p:PublishReadyToRun=true -p:IncludeNativeLibrariesForSelfExtract=true -p:EnableCompressionInSingleFile=true -p:PublishTrimmed=false -p:DebugType=none -p:DebugSymbols=false
```
...
```powershell
dotnet publish .\PaperTodo.csproj -c Release -r win-x64 --self-contained false -o 输出\PaperTodo-v1.6-win-x64-no-runtime-uncompressed\ -p:PublishSingleFile=true -p:PublishReadyToRun=false -p:EnableCompressionInSingleFile=false -p:PublishTrimmed=false -p:DebugType=none -p:DebugSymbols=false
```
