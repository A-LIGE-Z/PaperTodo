...
- **脚本胶囊**：笔记第一行写入 `!!power` / `!!powershell` 后，折叠胶囊会显示闪电图标，左键运行后续 PowerShell 脚本，右键可打开纸片继续编辑；粘贴带统一缩进的脚本时会自动整理缩进。
- 脚本胶囊新增 PowerShell 运行设置：可优先使用 PowerShell 7、隐藏运行窗口，并可开启 `!!powerflash` 复用常驻 PowerShell 进程。
- 优化脚本胶囊错误输出和临时脚本清理：普通 `!!power` 会稳定捕获错误详情，常驻模式投递失败或旧临时文件会自动清理。
